...
class wf_bundle_rate_addon_setup {
	...
	public function wf_woocommerce_shipping_pro_shipping_costs_method( $costs ) {
		...
		$allowed_calculation_mode = array( 'per_category', 'per_shipping_class' );
		if( empty($shipping_pro_settings) || ! isset($shipping_pro_settings['calc_per']) || !in_array($shipping_pro_settings['calc_per'],$allowed_calculation_mode))
			return $costs;
		
		$calc_mode = $shipping_pro_settings['calc_per'];
	...
	public function plugin_action_links( $links ) {
			$plugin_links = array(
				...
				'<a href="https://wordpress.org/support/plugin/bundle-rate-addon-for-woocommerce-shipping-pro" target="_blank">' . __( 'Support', 'wf_shipping_pro_addon' ) . '</a>',
			);
